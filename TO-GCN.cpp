...
void node_pair_generator_LD_or_TD(int opt) {
    ...
    if(opt == 0) { //considering both positive and negative correlations  LD+TD+
        for(int i=0; i<num_of_TFs; i++) {
            for(int j=0; j<num_of_genes; j++) {
    ...
    } else if(opt == 1) { //considering positive correlation in LD only  LD+TD0
        for(int i=0; i<num_of_TFs; i++) {
            for(int j=0; j<num_of_genes; j++) {
                if(strcmp(TF_exp_table[i].gene_ID, gene_exp_table[j].gene_ID) != 0) {
                    R_LD = r_calculator(i,j,0);
    ...
    } else if(opt == 2) { //considering positive correlation in TD only LD0TD+
        for(int i=0; i<num_of_TFs; i++) {
            for(int j=0; j<num_of_genes; j++) {
                if(strcmp(TF_exp_table[i].gene_ID, gene_exp_table[j].gene_ID) != 0) {
                    R_LD = r_calculator(i,j,0);
